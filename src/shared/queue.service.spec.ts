...
describe('QueueService', () => {
  ...
  describe('setupQueue', () => {
    it('should setup queue and dead letter queue successfully', async () => {
      ...
      expect(mockChannel.assertExchange).toHaveBeenCalledWith('dlx', 'topic', { durable: true });
      ...
      expect(mockChannel.assertQueue).toHaveBeenCalledWith('test-queue', expect.objectContaining({
        durable: true,
        messageTtl: 1000,
      ...
      expect(mockChannel.bindQueue).toHaveBeenCalledWith('test-queue', 'test-exchange', 'key');
    });
