...
describe('runQuantificationWithWorker', () => {
  ...
  beforeEach(() => {
    ...
    (Worker as unknown as ReturnType<typeof vi.fn>).mockImplementation(function() { return mockWorker; });
  });
  ...
  it('should reject on worker exit with code 0 but no result', async () => {
    ...
    await expect(promise).rejects.toThrow('SCRAM quantification worker exited before returning a result.');
  });
  ...
  it('should reject on worker exit with non-zero code', async () => {
    ...
    await expect(promise).rejects.toThrow('SCRAM quantification worker exited with code 1');
  });
});
