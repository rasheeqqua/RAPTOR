...
export class ProducerService implements OnApplicationBootstrap, OnApplicationShutdown {
  ...
  async onApplicationBootstrap(): Promise<void> {
    ...
    this.logger.debug("Initialized quant, distributed sequences, and adaptive sequences queues and ready to send messages");
  }
  ...
  public async createAndQueueAdaptiveSequenceBatch(
    ...
    const { sequenceRequests, sequenceJobIds } = this.sequenceExtractorService.extractSequenceRequests(quantRequest, parentJobId);
    this.logger.debug(`Creating adaptive batch job ${parentJobId} with ${sequenceRequests.length} sequences`);
    ...
    this.logger.debug(`Successfully queued ${sequenceRequests.length} adaptive sequence jobs for parent ${parentJobId}`);
    return sequenceJobIds;
  }
  ...
  async onApplicationShutdown(): Promise<void> {
    try {
      ...
      await this.channel?.deleteExchange(this.adaptiveSequencesQueueConfig.exchange.name);
