...
export class ConsumerService implements OnApplicationBootstrap, OnApplicationShutdown {
  ...
  constructor(
    ...
    this.adaptiveSequencesQueueConfig = this.queueConfigFactory.createAdaptiveSequencesJobQueueConfig();
  }
  ...
  private async consumeDistributedSequenceJobs(): Promise<void> {
      ...
      await this.channel?.consume(
        ...
        async (msg: ConsumeMessage | null) => {
          if (msg === null) throw new RpcException(`Null message from ${this.distributedSequencesQueueConfig.name}`);
          const modelsData = ((): NodeQuantRequest => {
            try {
              return typia.json.assertParse<NodeQuantRequest>(msg.content.toString());
            } catch (err) {
  ...
  private async handleRegularJob(nodeQuantRequest: NodeQuantRequest): Promise<void> {
    ...
    if (!isFileReference && result && typeof result === 'object') {
      ...
      analysisSeconds = resultObj.runtimeSummary?.analysisSeconds || resultObj.results?.runtimeSummary?.analysisSeconds;
      totalSeconds = resultObj.runtimeSummary?.totalSeconds || resultObj.results?.runtimeSummary?.totalSeconds || analysisSeconds;
  ...
  private async handleSequenceJob(nodeQuantRequest: NodeQuantRequest): Promise<void> {
    ...
    if (!isFileReference && result && typeof result === 'object') {
      ...
      if (resultObj.results?.sumOfProducts?.[0]) {
        ...
        probability = resultObj.results.initiatingEvents[0].sequences[0].probability;
  ...
  private async consumeAdaptiveSequenceJobs(): Promise<void> {
    ...
    await this.channel?.consume(
      ...
      async (msg: ConsumeMessage | null) => {
        if (msg === null) throw new RpcException(`Null msg from ${this.adaptiveSequencesQueueConfig.name}`);
  ...
  private async handleAdaptiveSequenceJob(nodeQuantRequest: NodeQuantRequest): Promise<void> {
    ...
    if (report && typeof report === "object" && !this.isFileResult(report)) {
      const runtimeSummary = (report as any).runtimeSummary ?? (report as any).results?.runtimeSummary;
      if (runtimeSummary) {
        analysisSeconds = runtimeSummary.analysisSeconds ?? undefined;
        totalSeconds = runtimeSummary.totalSeconds ?? runtimeSummary.analysisSeconds ?? undefined;
      }
    }
  ...
  private extractAdaptiveMetrics(report: any): {
    ...
    const sequence = Array.isArray(results.initiatingEvents) && results.initiatingEvents[0]?.sequences
    ...
    const rawOriginalProducts = sumOfProducts?.originalProducts
      ?? cutSets?.originalProducts;
    const originalProducts = typeof rawOriginalProducts === "number" ? rawOriginalProducts : undefined;

    // Final products count
    const rawProducts = sumOfProducts?.products
      ?? cutSets?.products
      ?? sequence?.products;
  ...
  private isFileResult(result: any): result is { type: "file"; path: string; size?: number | bigint; format?: string } {
    return Boolean(
      result &&
  ...
  private async storeQuantificationResult(jobId: string, result: any): Promise<string> {
    if (this.isFileResult(result)) {
      ...
      try {
        const sizeLabel = typeof result.size === "bigint" ? result.size.toString() : result.size ?? "unknown";
